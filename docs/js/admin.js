...
function doLogin(){
  ...
  if(u===ADMIN.u&&p===ADMIN.p){
    ...
    document.getElementById("login-err").textContent="Invalid credentials. Try admin / admin123";
...
function updateStats(){
  const ab=DB.filter(b=>b.status==="approved").length,pb=DB.filter(b=>b.status==="pending").length,rb=DB.filter(b=>b.status==="rejected").length;
  const ao=OPPORTUNITIES.filter(o=>o.status==="approved").length,po=OPPORTUNITIES.filter(o=>o.status==="pending").length;
  document.getElementById("st-ab").textContent=ab;document.getElementById("st-pb").textContent=pb;
  document.getElementById("st-rb").textContent=rb;document.getElementById("st-ao").textContent=ao;
  document.getElementById("st-po").textContent=po;document.getElementById("st-total").textContent=DB.length+OPPORTUNITIES.length;
  const badge=document.getElementById("atab-pending");
  const pendingTotal=pb+po;
  if(badge){badge.textContent=pendingTotal;badge.style.display=pendingTotal>0?"inline-flex":"none";}
}
...
function renderPending(){
  const list=DB.filter(b=>b.status==="pending"),el=document.getElementById("admin-pane-pending");
  ...
  el.innerHTML=list.map(b=>{
    ...
    return`<div class="pending-card"><div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:8px">
      <div><div style="font-weight:600;font-size:14px">${b.icon} ${escHtml(b.name)}</div></div>
      <span class="status-badge sb-pending">Pending</span></div>
      ...
      <div class="pending-actions">
        <button class="btn-approve" onclick="approveB(${idx})"><i class="fa-solid fa-check"></i> Approve & publish</button>
...
function reinstateB(idx){DB[idx].status="approved";DB[idx].lastUpdated=new Date().toISOString().slice(0,10);delete DB[idx].rejectReason;showToast("✓ Listing reinstated");renderAdminDash();}

function renderOppPending(){
  const list=OPPORTUNITIES.filter(o=>o.status==="pending"),el=document.getElementById("admin-pane-opp-pending");
  ...
  el.innerHTML=list.map(o=>{
    ...
    return`<div class="pending-card"><div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:8px">
      ...
      <div class="pending-meta">
        ...
        <span><i class="fa-solid fa-briefcase"></i>${escHtml(o.arrangement)}</span>
        <span><i class="fa-regular fa-calendar"></i>${escHtml(o.submittedAt||o.postedAt||"")}</span>
      </div>
      <p style="font-size:13px;color:var(--text-2);line-height:1.5;margin-bottom:10px">${escHtml((o.desc||"").substring(0,200))}…</p>
      ${o.salary?`<div style="font-size:12px;color:var(--text-3);margin-bottom:8px"><i class="fa-solid fa-dollar-sign"></i> ${escHtml(o.salary)}</div>`:""}
      ...
      <div class="pending-actions">
        <button class="btn-approve" onclick="approveO(${idx})"><i class="fa-solid fa-check"></i> Approve & publish</button>
...
function renderApproved(){
  const list=DB.filter(b=>b.status==="approved"),el=document.getElementById("admin-pane-approved");
  if(!list.length){el.innerHTML=`<div class="no-results"><h3>No approved listings.</h3></div>`;return;}
  el.innerHTML=`<div class="list-table">${list.map(b=>{const idx=DB.indexOf(b);
    return`<div class="list-row"><span style="font-size:20px">${b.icon}</span>
      <div style="flex:1;min-width:0"><div class="list-name">${escHtml(b.name)}</div><div class="list-sub">${escHtml(b.suburb)}, ${escHtml(b.state)} · ${escHtml(b.cat)} · ${relDate(b.lastUpdated)}</div></div>
      <span class="status-badge sb-approved">Active</span>
      <div class="list-actions">
        <button class="btn-icon" onclick="openBizModal(${b.id})" title="Preview"><i class="fa-solid fa-eye"></i></button>
        <button class="btn-icon btn-icon-red" onclick="removeB(${idx})" title="Remove"><i class="fa-solid fa-trash"></i></button>
      </div></div>`;
  }).join("")}</div>`;
}

function renderOppApproved(){
  const list=OPPORTUNITIES.filter(o=>o.status==="approved"),el=document.getElementById("admin-pane-opp-approved");
  if(!list.length){el.innerHTML=`<div class="no-results"><h3>No approved opportunities.</h3></div>`;return;}
  el.innerHTML=`<div class="list-table">${list.map(o=>{const idx=OPPORTUNITIES.indexOf(o);
    return`<div class="list-row"><span style="font-size:20px">${o.icon}</span>
      <div style="flex:1;min-width:0"><div class="list-name">${escHtml(o.title)}</div><div class="list-sub">${escHtml(o.org)} · ${escHtml(o.suburb)}, ${escHtml(o.state)} · ${escHtml(o.type)} · ${relDate(o.lastUpdated)}</div></div>
      ${oppTypeBadge(o.type)}
      <div class="list-actions">
        <button class="btn-icon" onclick="openOppDetail(${o.id})" title="Preview"><i class="fa-solid fa-eye"></i></button>
        <button class="btn-icon btn-icon-red" onclick="removeO(${idx})" title="Remove"><i class="fa-solid fa-trash"></i></button>
      </div></div>`;
  }).join("")}</div>`;
}

function removeO(idx){if(!confirm(`Remove "${OPPORTUNITIES[idx].title}"?`))return;OPPORTUNITIES[idx].status="rejected";OPPORTUNITIES[idx].rejectReason="Removed by admin";showToast("Opportunity removed","var(--amber-t)");renderAdminDash();}

function renderRejected(){
  const bl=DB.filter(b=>b.status==="rejected"),ol=OPPORTUNITIES.filter(o=>o.status==="rejected"),el=document.getElementById("admin-pane-rejected");
  if(!bl.length&&!ol.length){el.innerHTML=`<div class="no-results"><h3>No rejected items.</h3></div>`;return;}
  el.innerHTML=(bl.length?`<h3 style="font-size:14px;font-weight:600;margin-bottom:8px;color:var(--text-2)">Businesses</h3><div class="list-table" style="margin-bottom:1rem">${bl.map(b=>{const idx=DB.indexOf(b);return`<div class="list-row"><span style="font-size:20px">${b.icon}</span><div style="flex:1;min-width:0"><div class="list-name">${escHtml(b.name)}</div><div class="list-sub">${escHtml((b.rejectReason||"").substring(0,50))}</div></div><span class="status-badge sb-rejected">Rejected</span><button class="btn-icon btn-icon-green" onclick="reinstateB(${idx})" title="Reinstate"><i class="fa-solid fa-rotate-left"></i></button></div>`;}).join("")}</div>`:"")
  +(ol.length?`<h3 style="font-size:14px;font-weight:600;margin-bottom:8px;color:var(--text-2)">Opportunities</h3><div class="list-table">${ol.map(o=>{return`<div class="list-row"><span style="font-size:20px">${o.icon}</span><div style="flex:1;min-width:0"><div class="list-name">${escHtml(o.title)}</div><div class="list-sub">${escHtml(o.org)} · ${escHtml((o.rejectReason||"").substring(0,40))}</div></div><span class="status-badge sb-rejected">Rejected</span></div>`;}).join("")}</div>`:"");
}
...
function renderAnalytics(){
  ...
  const maxS=Math.max(...Object.values(sc),1),maxI=Math.max(...Object.values(ic),1),maxT=Math.max(...Object.values(tc),1);
  el.innerHTML=`<div class="analytics-grid">
    <div class="analytics-card"><h3>Businesses by state</h3>${Object.entries(sc).sort((a,b)=>b[1]-a[1]).map(([s,n])=>`<div class="bar-row"><span class="bar-lbl">${escHtml(s)}</span><div class="bar-track"><div class="bar-fill" style="width:${Math.round(n/maxS*100)}%"></div></div><span class="bar-val">${n}</span></div>`).join("")}</div>
    <div class="analytics-card"><h3>Top industries</h3>${Object.entries(ic).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([ind,n])=>`<div class="bar-row"><span class="bar-lbl" title="${escHtml(ind)}">${escHtml(ind)}</span><div class="bar-track"><div class="bar-fill" style="width:${Math.round(n/maxI*100)}%"></div></div><span class="bar-val">${n}</span></div>`).join("")}</div>
    <div class="analytics-card"><h3>Opportunities by type</h3>${Object.entries(tc).sort((a,b)=>b[1]-a[1]).map(([t,n])=>`<div class="bar-row"><span class="bar-lbl">${escHtml(t)}</span><div class="bar-track"><div class="bar-fill" style="width:${Math.round(n/maxT*100)}%"></div></div><span class="bar-val">${n}</span></div>`).join("")}</div>
  </div>`;
}
